-- ============================================
-- Supabase 完整数据库初始化（修复版）
-- 使用 now() 代替 CURRENT_TIMESTAMP，避免语法错误
-- 复制到 SQL Editor → 执行
-- ============================================

-- 如需全新开始，取消下面注释（会删除所有数据）
-- DROP TABLE IF EXISTS arc_comments, arcs, images, sections, characters, replies, comments, posts, users CASCADE;
...
CREATE INDEX IF NOT EXISTS idx_arc_comments_arc_id ON arc_comments(arc_id);
...
('admin', '$2b$10$1tyv7b4fYxe8amh1VzbsOuOfrusFPZFsYUWcbJZGswgv9bSx.mVvG', 'admin', false)
...
CREATE POLICY "images_all" ON images FOR ALL USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS "arcs_all" ON arcs;
CREATE POLICY "arcs_all" ON arcs FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "arc_comments_all" ON arc_comments;
CREATE POLICY "arc_comments_all" ON arc_comments FOR ALL USING (true) WITH CHECK (true);
